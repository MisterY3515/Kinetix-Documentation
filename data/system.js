App.register('system', {
    ...
    example: `// OS Detection with compile-time branch elimination
if system.os.isWindows() {
    print("Running on Windows!")
} else if system.os.isLinux() {
    print("Running on Linux!")
} else if system.os.isMac() {
...
let cores = system.get_hardware_info().cpu_cores`,
    methods: [
        ...
        { name: 'os.isWindows', ret: 'bool', params: '()', desc: 'Returns true if the host OS is Windows.', details: 'Resolved at compile-time via Rust <code>cfg!(windows)</code>. When used inside <code>if</code> conditions, the compiler performs constant folding to eliminate dead branches entirely from the generated bytecode.', example: 'if system.os.isWindows() { print("Windows!") }', implemented: 'v0.0.8 (23)' },
        { name: 'os.isLinux', ret: 'bool', params: '()', desc: 'Returns true if the host OS is Linux.', details: 'Resolved at compile-time via Rust <code>cfg!(target_os = "linux")</code>. Supports compile-time branch elimination.', example: 'if system.os.isLinux() { print("Linux!") }', implemented: 'v0.0.8 (23)' },
        { name: 'os.isMac', ret: 'bool', params: '()', desc: 'Returns true if the host OS is macOS.', details: 'Resolved at compile-time via Rust <code>cfg!(target_os = "macos")</code>. Supports compile-time branch elimination.', example: 'if system.os.isMac() { print("macOS!") }', implemented: 'v0.0.8 (23)' },
        { name: 'os.arch', ret: 'String', params: '()', desc: 'Returns the host CPU architecture.', details: 'Examples: "x86_64", "aarch64". Zero-panic API.', example: 'println(system.os.arch())', implemented: 'v0.0.8 (23)' },
        { name: 'exec', ret: 'Result', params: '(cmd: String)', desc: 'Executes a native OS command.', details: 'Spawns a child process and blocks until completion. Returns a Result wrapped dictionary containing stdout, stderr, and status. Fails cleanly without panicking VM.', example: 'let out = system.exec("echo test")\nprintln(out.ok.stdout)', implemented: 'v0.0.8 (23)' },
        ...
        { name: 'thread.spawn', ret: 'Thread', params: '(fn)', desc: 'Spawns a new VM thread executing the parameter closure.', details: 'Placeholder. Real RAII threading scheduled for subsequent release.', example: 'system.thread.spawn(fn() { ... })', implemented: 'v0.0.8 (23)' },
        { name: 'thread.join', ret: 'Result', params: '(thread)', desc: 'Blocks until the specified thread concludes.', details: 'Placeholder for true capability.', example: 'system.thread.join(t)', implemented: 'v0.0.8 (23)' },
